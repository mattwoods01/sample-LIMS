...
def index(request):
    """
    Get instruments API route from labguru and save as dataframe
    """
    ...
    # instrument_ids = pd.DataFrame(instrument_catalogue)

    # print(instrument_ids)

    # instrument_ls = []
    # for i, row in instrument_ids.iterrows():
    #     indv_instrument = [requests.get(f'https://thermofisher.labguru.com/api/v1/instruments/{row["id"]}', json=payload, headers=headers).json()]
    #     instrument_ls.append(indv_instrument)
    #     print(instrument_ls)

    # instrument_df = pd.DataFrame([instrument_ls])

    # print(instrument_df)

    def extract_equipment_data(equipment):
        """
        extract equipment data by getting various keys from labguru equipment df
        """
    ...
    return render(request, 'man_maintenance_updater.html', context)
